...
function sum(snailNumberA, snailNumberB) {
    return ['[', ...snailNumberA, ',', ...snailNumberB, ']'];
}
...
inputStream.on('line', (line) => {
    if (finalNumber == undefined) {
        finalNumber = line.split('');
    } else {
        finalNumber = sum(finalNumber, line.split(''));
        finalNumber = reduce(finalNumber);
    }
});

inputStream.on('close', () => {
    const magnitude = calculateMagnitudeForPair(...getPair(finalNumber));
